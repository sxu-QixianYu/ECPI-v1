...
mopopenaltyList = [0.5,1.0]
cqlpenaltyList = [1.3,1.4,1.5]

class EnsembleDynamicsCopy(BaseDynamics):
    def __init__(
        self,
        model: nn.Module,
        ...
        penalty_coef_targetq: float = 0.0,
        ...
        self._penalty_coef_targetq = penalty_coef_targetq
    ...
    def get_uncertainty(
        ...
        #     next_obses_mean = mean[..., :-1]
        #     penalty = np.sqrt(next_obses_mean.var(0).mean(1))
        # elif self._uncertainty_mode == "pairwise-diff-mean":
        #     next_obses_mean = mean[..., :-1]
        #     next_obs_mean = np.mean(next_obses_mean, axis=0)
        #     diff = next_obses_mean - next_obs_mean
        #     penalty = np.mean(np.linalg.norm(diff, axis=2), axis=0)
        else:
            raise ValueError
        penalty = np.expand_dims(penalty, 1).astype(np.float32)
        # penaltyW = [0.6 / i if i > 0.6 else [1,] for i in penalty]
