...
class MBPolicyTrainer:
    ...
    def train(self) -> Dict[str, float]:
        ...
        for e in range(1, self._epoch + 1):
            ...
            if e % 30 == 0:
                ...
                real_reward = eval_info["eval/real_reward"]

                self.logger.logkv("eval/episode_length", ep_length_mean)
                self.logger.logkv("eval/episode_length_std", ep_length_std)
                self.logger.logkv("real_reward", real_reward)
        ...
        #     # i_test += 1
        #     with open("/home/uas/yqx/off-kit/mobileData/test-result-t" + str(i_test+1) + ".pkl", "wb") as f:
        #         pickle.dump(t_q.cpu().detach().numpy(), f)
        #
        # with open("/home/uas/yqx/off-kit/mc+pc/test-result-mobile.pkl", "wb") as f:
    ...
    def _evaluate(self) -> Dict[str, List[float]]:
        ...
        eval_ep_info_buffer = []
        num_episodes = 0
        episode_reward, episode_length, real_reward = 0, 0, 0
        n = 0
        while num_episodes < self._eval_episodes:
            action = self.policy.select_action(obs.reshape(1, -1), deterministic=True)
            next_obs, reward, terminal, _ = self.eval_env.step(action.flatten())
            episode_reward += reward
            real_reward += reward*(0.99**n)
            episode_length += 1
            obs = next_obs
            n +=1
            ...
            if terminal:
                ...
                obs = self.eval_env.reset()
        real_reward = real_reward/num_episodes

        return {
            "eval/episode_reward": [ep_info["episode_reward"] for ep_info in eval_ep_info_buffer],
            "eval/episode_length": [ep_info["episode_length"] for ep_info in eval_ep_info_buffer],
            "eval/real_reward": real_reward
        }