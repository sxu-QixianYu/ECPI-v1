...
class ReplayBuffer:
    ...
    def sample_index(self, batch_index: list) -> Dict[str, np.ndarray]:
        return {
            "observations": torch.tensor(self.observations[batch_index]).to(self.device),
            "actions": torch.tensor(self.actions[batch_index]).to(self.device),
            "next_observations": torch.tensor(self.next_observations[batch_index]).to(self.device),
            "terminals": torch.tensor(self.terminals[batch_index]).to(self.device),
            "rewards": torch.tensor(self.rewards[batch_index]).to(self.device)
        }